...
feature 'Only Admin can create condition for the mortgage' do
  ...
  context 'Admin' do
    ...
    scenario 'can create condition for mortgage' do
      fill_in t('activerecord.attributes.condition.interest_rate'), with: '500'
      fill_in t('activerecord.attributes.condition.value_interest_rate_with_commision'), with: 2.5
      fill_in t('activerecord.attributes.condition.max_loan_amount'), with: '300000'
      fill_in t('activerecord.attributes.condition.value_max_loan_amount'), with: 50
      fill_in t('activerecord.attributes.condition.note'), with: 'blablabla'
      click_on t('conditions.edit.add_condition')
  ...
  scenario 'No admin can not see fields for entering conditions' do
    ...
    expect(page).to_not have_content t('activerecord.attributes.condition.interest_rate')
    expect(page).to_not have_content t('activerecord.attributes.condition.max_loan_amount')
  end
